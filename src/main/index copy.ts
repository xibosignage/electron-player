...
const mainFunctions = {
  run: async (win: BrowserWindow, config: Config) => {
    ...
    win.webContents.send('configure', { config: config, xmds: xmds });
...
const init = (win) => {
  ...
  config.load().then(async() => {
    ...
    if (!config.isConfigured()) {
      ...
      ipcMain.on('xmds-try-register', async (_event) => {
        console.log('xmds-try-register: ');
